...
// ѕолучаем настройки пид регул€торов
void tension_init()
{
...
// ¬ызываетс€ таймером.
// ”становка нат€жени€ ленты в зависимомти от режима.
...
// ≈сли это воспроизведение - будем подстраивать нат€жение на подмоточном узле относительно скорости вращени€ боковых узлов
// ѕереписать.... родилось в результате дебага и отладки..
...
// ≈сли это воспроизведение - будем подстраивать нат€жение на подающем узле
static void stabilization_play()
{
...
// ≈сли это перемотка вперед - будем подстраивать нат€жение на подающем узле
static void stabilization_forward()
{
	servo_list[SERVO_LEFT].need_angle = computePID(
		kinematics_mode.tension,
...
// ≈сли это перемотка назад - будем подстраивать нат€жение на подающем узле
static void stabilization_rewind()
{
